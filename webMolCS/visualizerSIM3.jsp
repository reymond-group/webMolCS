	...
	<script>
		...
		function init() {
			...
			for(var i=0;i<(treePoints1.length-1);i++)
			{
				...
				var vec1= new THREE.Vector3(xyz1[0],xyz1[1],xyz1[2]);
				var vec2= new THREE.Vector3(xyz2[0],xyz2[1],xyz2[2]);

				var g = new THREE.Geometry();
                        	g.vertices = [vec1, vec2];

				var col=simToQuery2_COLOR[pntIdx1].split("_");
				var clr = new THREE.Color();
				clr.setRGB(parseInt(col[0])/255, parseInt(col[1])/255, parseInt(col[2])/255);

				var m = new THREE.LineBasicMaterial(
                        	{ color: clr, opacity: 1, linewidth: 3}
                        	);
			...
			//var g, line;
			//var lines = new THREE.Object3D();
			//var line1 = [new THREE.Vector3(186,132,137), new THREE.Vector3(194,94,184)];
	    		//var line2 = [new THREE.Vector3(0, 300, -3), new THREE.Vector3(300, 0, -2)];
			//var m = new THREE.LineBasicMaterial({
	        	//color: 0xff0000,
	        	//linewidth:2 
	    		//});
