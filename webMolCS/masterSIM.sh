...
##GO TO THE FINGERPRINT FOLDER AND DO MAP CALCULATION
...
java -cp ../../dist/SimMapPortalGenRator.jar calculateSimFP.screen031${fp} REF.smi REF.smi DB.smi DB.smi.simfp
...
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.A_CalcMqnAndProps -i $i -o MQNPROP/$(basename $i) -m -s DATS/$(basename $i).sums

done
####################################################################################
#2))

java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.B_MergeSumsToAvg $name.avgs DATS/${name}*.sums

####################################################################################
#3) mean centralize
for i in MQNPROP/${name}*
do
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.C_MeanCentralSumCov -i $i -o MC/$(basename $i) -a $name.avgs -c DATS/$(basename $i).covsums
done

####################################################################################
#4) merge

java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.D_MergeCovCalcEigen $name.eWeV DATS/${name}*.covsums
...
for i in MC/${name}*
do
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.E_TransposeToPC123 -i $i -o TRANS/$(basename $i) -e $name.eWeV -m DATS/$(basename $i).minmax
done

####################################################################################
#5) merge
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.F_MergeMinMax $name.totminmax DATS/${name}*.minmax
####################################################################################

#6) make partial maps
for i in TRANS/${name}*
do
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.G_CreatePartMaps PARTMAPS/$(basename $i) $name.totminmax $size $i
done
####################################################################################
#7) merge maps to big ones
for i in simToQuery1 ActualValues simToQuery2 maxSimToRef
do
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.H_MapMerger ASFMAPS/${name}_merge.$i.asf PARTMAPS/${name}*.$i.*
done
####################################################################################
...
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.I_Binning BINS $name.totminmax $start $max $size $size $size TRANS/*

done
####################################################################################
# 10) Merge Average Files
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.J_MergeAverageFiles $name.tmpAVG X_*.avg
####################################################################################
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.M_ColorCoding -i ASFMAPS/${name}_merge.maxSimToRef.asf -o M_ColoredPoints/maxSimToRef.binsColor -m 1,0.2,4,0,1,0 -c 240,360

java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.M_ColorCoding -i ASFMAPS/${name}_merge.simToQuery1.asf -o M_ColoredPoints/simToQuery1.binsColor -m 1,0,4,0,1,0 -c 240,360

java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.M_ColorCoding -i ASFMAPS/${name}_merge.simToQuery2.asf -o M_ColoredPoints/simToQuery2.binsColor -m 1,0,4,0,1,0 -c 240,360
...
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.M_ColorCoding -i ASFMAPS/${name}_merge.ActualValues.asf -o M_ColoredPoints/ActualValues.binsColor -m $HIGH,$LOW,4000000,0,1,0 -c 240,360
####################################################################################
for i in simToQuery1 ActualValues simToQuery2 maxSimToRef
do
java -Xmx4000M -cp ../../dist/SimMapPortalGenRator.jar ThreeDOrganizer.N_finalDataFiles M_ColoredPoints/${i}.binsColor $name.tmpAVG ${i}/$name ${i}/$name $name
done
####################################################################################
rm X_*.avg
####################################################################################
